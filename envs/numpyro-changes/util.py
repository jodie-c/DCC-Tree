...
def log_prob( #TODO
    ...
    def single_logprob(samples):
        ...
        #         log_prob = jnp.sum(log_prob)
        #         log_joint = log_joint + log_prob
        return {name: site["fn"].log_prob(site["value"]) for name, site in model_trace.items() if site["type"] == "sample"}#{'log_prob':log_joint}
    ...
    batch_size = int(np.prod(batch_shape))
    chunk_size = batch_size if parallel else 1
    return soft_vmap(single_logprob, posterior_samples, len(batch_shape), chunk_size)
