...
def warmup_adapter(
    ...
    if(num_adapt_steps > 0): # TODO
        len_h_window = adaptation_schedule[-2].end - adaptation_schedule[1].start ## TODO
        h_init = adapt_h_info['h_init'] ## TODO
        h_final = adapt_h_info['h_final'] ## TODO
    ...
    def init_fn(
        ...
        ### TODO INITIALISED H VALUE/STEP HERE
        h_step = (h_init-h_final)/(len_h_window-100)
        h_state = h_init, h_step
    ...
    def update_fn(t, accept_prob, z_info, state):
        ...
        ### TODO ADDED FUNCTIONS HERE TO UPDATE VALUE FOR H
        (h_value,h_step) = h_state
        h_window = (adaptation_schedule[1].start <= t) & (t <= (adaptation_schedule[-2].end-100))
        # h_window = ((num_windows - 2) <= window_idx) & (window_idx < (num_windows - 1))
        h_value = cond(
            h_window,
            h_value,
            lambda h: h - h_step,
